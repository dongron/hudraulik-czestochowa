...
- [ ] **T1.1** Delete `frontend/app/components/Header.tsx` and `frontend/app/components/Footer.tsx`
...
- [ ] **T2.1** `git mv` `LandingNav.tsx` → `SiteHeader.tsx`; rename default export `LandingNav` → `SiteHeader`
  - **Done when:** file renamed, export is `SiteHeader`, no markup/behavior change.
- [ ] **T2.2** `git mv` `LandingFooter.tsx` → `SiteFooter.tsx`; rename default export `LandingFooter` → `SiteFooter`
  - **Done when:** file renamed, export is `SiteFooter`, no markup/behavior change.
- [ ] **T2.3** Update `app/page.tsx` imports/usages to `SiteHeader` / `SiteFooter`
...
- [ ] **T3.1** `app/layout.tsx`: fetch settings in the component body
...
- [ ] **T3.2** `app/layout.tsx`: render `<SiteHeader settings={settings} />` before `<main>{children}</main>` and `<SiteFooter settings={settings} />` after, inside the existing `<section className="min-h-screen">`
  - **Done when:** layout owns the single `<main>` and both chrome components.
- [ ] **T3.3** `app/page.tsx`: remove `SiteHeader`/`SiteFooter` renders + imports; remove the page's own `<main>` wrapper (return a fragment); keep `LocalBusinessSchema` + the page-builder `switch`
...
- [ ] **T4.1** `SiteHeader.tsx`: change `NAV_LINKS` hrefs `#uslugi/#opinie/#o-mnie/#kontakt` → `/#…`; logo href `#top` → `/`
  - Preserve all `data-ga-event` / `data-ga-section` attributes.
  - **Done when:** every header link is root-relative; logo → `/`.
- [ ] **T4.2** `SiteFooter.tsx`: change footer nav anchor hrefs `#…` → `/#…`
  - Preserve all `data-ga-event` / `data-ga-section` attributes.
...
- [ ] **TF.1** Full gate green: `pnpm run type-check`, `pnpm run lint`, `pnpm --filter frontend test` (AC-7, AC-8)
- [ ] **TF.2** `grep -rn "components/Header\|components/Footer\|LandingNav\|LandingFooter" frontend/app` → no matches (AC-6)
- [ ] **TF.3** `pnpm run format` (Prettier) on touched files
- [ ] **TF.4** Manual 3-route check per plan Verification Gate
- [ ] **TF.5** Confirm `studio/`, `sanity.types.ts` untouched; `git status` shows only intended files
